...
def obtener_precios(ticker, start_date, end_date, interval='1h'):
    try:
...
def calcular_emas(prices):
    try:
        if len(prices) < 200:  # Verificar que haya suficientes datos
...
def detectar_cruce_ema_4hs(ema50, ema200):
    if ema50 is None or ema200 is None or len(ema50) < 3 or len(ema200) < 3:
        return None

    cruce_alcista = int(
        (ema50.iloc[-3] < ema200.iloc[-3]) and (ema50.iloc[-2] >= ema200.iloc[-2])
    ) + int(
        (ema50.iloc[-2] < ema200.iloc[-2]) and (ema50.iloc[-1] >= ema200.iloc[-1])
    )
...
def calcular_emas_4h_1d(prices, periodo, ticker=None):
    try:
...
async def analizar_multiples_acciones_1h(tickers, resultados):
    cruces_alcistas_ema = []
    cruces_bajistas_ema = []
    ...
    print("Ejecutando análisis en 1H")
    ...
    async def procesar_ticker(ticker):
        ...
        try:
            ...
            prices = obtener_precios(ticker, start_date, end_date, interval='1h')
            if prices is None or prices.empty:
                sin_datos.append(ticker)
                return

            if len(prices) < 200:
                print(f"\033[91m{ticker} tiene {len(prices)} velas de 1H. Necesarias al menos 200 para EMAs.\033[0m")
                sin_datos.append(ticker)
                return
            ...
            if condicion_volumen:
                volumen_fuerte_1h.append(ticker)
            ...
            print(f"\033[91mError al procesar {ticker} en 1H: {e}\033[0m")
    ...
    print("\n\033[92mResultados del análisis 1H:\033[0m")

    print("\n\033[94mCruces de EMAs 50/200 1H:\033[0m")
    print(f"Cruces alcistas: {', '.join(cruces_alcistas_ema) if cruces_alcistas_ema else 'Ninguno'}")
    print(f"Cruces bajistas: {', '.join(cruces_bajistas_ema) if cruces_bajistas_ema else 'Ninguno'}")

    print("\n\033[94mCruces de MACD 1H:\033[0m")
    print(f"Cruces alcistas: {', '.join(cruces_alcistas_macd) if cruces_alcistas_macd else 'Ninguno'}")
    print(f"Cruces bajistas: {', '.join(cruces_bajistas_macd) if cruces_bajistas_macd else 'Ninguno'}")

    print("\n\033[94mVolumen Fuerte 1H:\033[0m")
    print(f"{', '.join(volumen_fuerte_1h) if volumen_fuerte_1h else 'Ninguno'}")
    ...
    if not sin_datos and not sin_procesar:
        ...
        if sin_datos:
            print(f"\033[91mNo se encontraron datos: {', '.join(sin_datos)}\033[0m")
        if sin_procesar:
            print(f"\033[91mNo se pudo procesar: {', '.join(sin_procesar)}\033[0m")
...
async def analizar_multiples_acciones_4h(tickers, resultados):
    cruces_alcistas_ema = []
    cruces_bajistas_ema = []
    cruces_alcistas_macd = []
    cruces_bajistas_macd = []
    ...
    print("Ejecutando análisis en 4H")
    actualizar_progreso(total_procesados, total_tickers)
    ...
    async def procesar_ticker(ticker):
        ...
        try:
            end_date = pd.Timestamp.now()
            start_date = end_date - pd.DateOffset(days=120)  # Más días para asegurar datos al re-muestrear
            ...
            # Aplanar columnas si tienen MultiIndex (por ejemplo: ('Close', 'AAPL'))
            if isinstance(prices.columns, pd.MultiIndex):
                prices.columns = prices.columns.get_level_values(0)

            # Remuestrear a 4H
            prices = prices.resample('4h').agg({
                'Open': 'first',
                'High': 'max',
                'Low': 'min',
                'Close': 'last',
            ...
            if len(prices) < 200:
                print(f"\033[91m{ticker} tiene {len(prices)} velas de 4H. Necesarias al menos 200 para EMAs.\033[0m")
                sin_datos.append(ticker)
                return
            ...
            # EMAs
            ema50, ema200 = calcular_emas(prices)
            cruce_ema = detectar_cruce_ema_4hs(ema50, ema200)
            ...
            condicion_volumen, volumen_actual, media_volumen_actual = confirmar_volumen(prices)
            if condicion_volumen:
                volumen_fuerte_4h.append(ticker)
            ...
            sin_procesar.append(ticker)
            print(f"\033[91mError al procesar {ticker} en 4H: {e}\033[0m")
    ...
    print("\n\033[92mResultados del análisis 4H:\033[0m")

    print("\n\033[94mCruces de EMAs 50/200 4H:\033[0m")
    print(f"Cruces alcistas: {', '.join(cruces_alcistas_ema) if cruces_alcistas_ema else 'Ninguno'}")
    print(f"Cruces bajistas: {', '.join(cruces_bajistas_ema) if cruces_bajistas_ema else 'Ninguno'}")

    print("\n\033[94mCruces de MACD 4H:\033[0m")
    print(f"Cruces alcistas: {', '.join(cruces_alcistas_macd) if cruces_alcistas_macd else 'Ninguno'}")
    print(f"Cruces bajistas: {', '.join(cruces_bajistas_macd) if cruces_bajistas_macd else 'Ninguno'}")

    print("\n\033[94mVolumen Fuerte 4H:\033[0m")
    print(f"{', '.join(volumen_fuerte_4h) if volumen_fuerte_4h else 'Ninguno'}")
...
async def verificar_coincidencias_generales(
    resultados_1h, resultados_4h, resultados_1d,
    volumen_fuerte_1h, volumen_fuerte_4h, volumen_fuerte_1d,
    rsi_sobrevendido_1h, rsi_sobrevendido_4h, rsi_sobrevendido_1d,
    rsi_sobrecomprado_1h, rsi_sobrecomprado_4h, rsi_sobrecomprado_1d,
    adx_fuerte_1h, adx_fuerte_4h, adx_fuerte_1d
):
    ...
    for datos, intervalo in [(resultados_1h, "1h"), (resultados_4h, "4h"), (resultados_1d, "1d")]:
    ...
    for ticker in volumen_fuerte_1h:
        agregar_volumen_neutral(ticker, "1h")
    for ticker in volumen_fuerte_4h:
        agregar_volumen_neutral(ticker, "4h")
    ...
    for ticker in rsi_sobrevendido_1h:
        agregar_rsi_neutral(ticker, "1h", "alcista")
    for ticker in rsi_sobrevendido_4h:
        agregar_rsi_neutral(ticker, "4h", "alcista")
    for ticker in rsi_sobrevendido_1d:
        agregar_rsi_neutral(ticker, "1d", "alcista")

    for ticker in rsi_sobrecomprado_1h:
        agregar_rsi_neutral(ticker, "1h", "bajista")
    for ticker in rsi_sobrecomprado_4h:
        agregar_rsi_neutral(ticker, "4h", "bajista")
    ...
    for ticker in adx_fuerte_1h:
        agregar_adx_neutral(ticker, "1h")
    for ticker in adx_fuerte_4h:
        agregar_adx_neutral(ticker, "4h")
    ...
    def contradice_rsi(ticker, tipo):
        if tipo == "bajista":
            return (
                ticker in rsi_sobrevendido_1h or
                ticker in rsi_sobrevendido_4h or
                ticker in rsi_sobrevendido_1d
            )
        elif tipo == "alcista":
            return (
                ticker in rsi_sobrecomprado_1h or
                ticker in rsi_sobrecomprado_4h or
                ticker in rsi_sobrecomprado_1d
            )
        return False
...
async def main():
    while True:
        ...
        print("1. Análisis intervalo 1h")
        print("2. Análisis intervalo 4h")
        print("3. Análisis intervalo 1d")
        ...
        if opcion == '1':
            ...
            resultados_1h, volumen_fuerte_1h = await analizar_multiples_acciones_1h(
                tickers_seleccionados,
                {'alcistas': [], 'bajistas': []}
            )
            print("\nAnálisis 1h completado.")

        elif opcion == '2':
            print("\nIniciando análisis de 1 semana...\n")
            resultados_4h, volumen_fuerte_4h = await analizar_multiples_acciones_4h(
                tickers_seleccionados,
                {'alcistas': [], 'bajistas': []}
            )
            ...
            await verificar_coincidencias_generales(
                resultados_1h, resultados_4h, resultados_1d,
                volumen_fuerte_1h, volumen_fuerte_4h, volumen_fuerte_1d,
                resultados_1h.get('rsi_sobrevendido', []),
                resultados_4h.get('rsi_sobrevendido', []),
                resultados_1d.get('rsi_sobrevendido', []),
                resultados_1h.get('rsi_sobrecomprado', []),
                resultados_4h.get('rsi_sobrecomprado', []),
                resultados_1d.get('rsi_sobrecomprado', []),
                resultados_1h.get('adx_fuerte', []),
                resultados_4h.get('adx_fuerte', []),
